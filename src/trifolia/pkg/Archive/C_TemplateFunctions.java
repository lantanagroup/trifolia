...
//TEST 1: View an existing Template
	...
	{
	FindTemplate("eMeasure Reference QDM", "urn:oid:2.16.840.1.113883.10.20.24.3.97", "QRDA Category I Release 2");	 
	}
    ...
    // Confirm the Template Viewer opens
	    ...
	    wait2.until(ExpectedConditions.textToBePresentInElementLocated(By.xpath("/html/body/div[2]/div/div/div[2]/div/h4"), templateOID));
	 	assertTrue("Could not find \"Template OID\" on page.", driver.findElement(By.cssSelector("BODY")).getText().indexOf(templateOID) >= 0);
    ...
    // Thread.sleep(2000);
    WebDriverWait wait8 = new WebDriverWait(driver, 60);
    WebElement element8 = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@id=\"ViewTemplate\"]/ul/li[2]/a")));
    ...
    if (permissionUserName == "lcg.admin") 
    {
	    ...
	    // Return to the Template Listing page and confirm the correct form appears
    ...
    else
    {
	    ...
	    WebElement element15 = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("BrowseTemplates")));
	     
	    // Confirm the form is available
	    WebDriverWait wait16 = new WebDriverWait(driver, 60);
	    WebElement element16 = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("/html/body/div[2]/div/div/table/thead/tr[2]/td[5]/button")));
  ...
  @Test
	  ...
	  if (permissionUserName == "lcg.admin")
	  {
	   ...
	   // Move the Template
		  ...
		  WebDriverWait wait1 = new WebDriverWait(driver, 60);
		  WebElement element1 = wait1.until(ExpectedConditions.elementToBeClickable(By.xpath("/html/body/div[2]/div/div/div[2]/div[2]/button[1]")));
    ...
    // Accept the Delete Confirmation
      ...
      //Confirm the Alert appears
	   ...
	   wait0.until(ExpectedConditions.alertIsPresent());
	    ...
	    // Switch the driver context to the "Delete Template" alert
	    Alert alertDialog = driver.switchTo().alert();
	    // Get the alert text
	    String alertText = alertDialog.getText();
	    // assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Are you absolutely sure you want to delete this template?[\\s\\S]*$"));
	    // Click the OK button on the alert.
	    alertDialog.accept();
	    Thread.sleep(1000);
	    // assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Successfully deleted template.[\\s\\S]*$"));
	    alertDialog.accept();

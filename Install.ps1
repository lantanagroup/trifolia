...
if (!$NoMigrate) {
    ...
    Set-location -Path $migrateBin -PassThru

    $migrateCommand = '& .\migrate.exe "Trifolia.DB.dll" "/startupDirectory:$migrateBin" "/startUpConfigurationFile:$destAppWebConfigPath" "/verbose"'
    Write-Host "Running: " $migrateCommand

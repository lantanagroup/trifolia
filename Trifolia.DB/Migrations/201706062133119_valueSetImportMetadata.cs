namespace Trifolia.DB.Migrations
{
    ...
    public partial class valueSetImportMetadata : DbMigration
    {
        public override void Up()
        {
            AddColumn("dbo.valueset", "importSource", c => c.Int());
            AddColumn("dbo.valueset", "importSourceId", c => c.String());
...
BEGIN
	...
	CREATE TABLE #valuesets (
		...
		importSourceId NVARCHAR(MAX) NULL)
	...
	SELECT DISTINCT
		...
		vs.importSourceId
	FROM valueset vs
	LEFT JOIN valueset_identifier vsi on vs.id = vsi.valueSetId
	WHERE
		vs.code LIKE @searchTextAny
		OR vs.name LIKE @searchTextAny
		OR vsi.identifier LIKE @searchTextAny
...
");
        ...
        {
            DropColumn("dbo.valueset", "importSourceId");
            DropColumn("dbo.valueset", "importSource");
